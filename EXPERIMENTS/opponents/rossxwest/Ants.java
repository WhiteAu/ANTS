...
public class Ants {
    ...
    public void update(Ilk ilk, Tile tile) {
        if (ilk == ilk.FOOD) foodTiles.add(tile);
        map[tile.getRow()][tile.getCol()] = ilk;
    }
    
    public void updateAnt(Tile tile, int owner) {
        map[tile.getRow()][tile.getCol()] = (owner == 0) ? Ilk.MY_ANT : Ilk.ENEMY_ANT;
        
        Ant a = new Ant(tile.getRow(), tile.getCol(), owner);
