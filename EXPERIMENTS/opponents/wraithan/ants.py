...
class Ants():
    def __init__(self):
        ...
        self.base_map = None
        self.hill_list = {}
        self.known_enemy_hill_list = set()
        self.ant_list = {}
    ...
    def setup(self, data):
        ...
        self.weighted_map = [[None for col in range(self.cols)]
                             for row in range(self.rows)]
    ...
    def update(self, data):
        ...
        for row, col in self.food_list:
            self.map[row][col] = LAND
        self.food_list = []
        self.weighted_map = [[None for col in range(self.cols) if col != -1]
                             for row in range(self.rows)]
            ...
            if len(line) > 0:
                ...
                if len(tokens) >= 3:
                    ...
                    if tokens[0] == 'w':
                        ...
                        self.weighted_map[row][col] = WATER_WEIGHT
                    elif tokens[0] == 'f':
                        self.map[row][col] = FOOD
                        self.weighted_map[row][col] = FOOD_WEIGHT
                        self.food_list.append(loc)
                        self.poi_list.append(loc)
                    else:
                        owner = int(tokens[3])
                        if tokens[0] == 'a':
                            self.map[row][col] = owner
                            # self.weighted_map[row][col] = ANT_WEIGHT
                            self.ant_list[loc] = owner
                        elif tokens[0] == 'd':
                            ...
                            self.dead_list[loc].append(owner)
                        elif tokens[0] == 'h':
                            owner = int(tokens[3])
                            self.hill_list[loc] = owner
                            if owner != MY_ANT:
                                self.poi_list.append(loc)
                                self.weighted_map[row][col] = HILL_WEIGHT
                                self.known_enemy_hill_list.add(loc)
                            else:
                                self.weighted_map[row][col] = -1
    ...
    def render_text_weighted_map(self):
        'return a pretty string representing the map'
        tmp = '<table>\n'
        def render(meh):
            if meh is None:
                ...
                return str('<td>%d</td>'% meh)
        for row in self.weighted_map:
            tmp += '<tr>%s</tr>\n' % ''.join([render(col) for col in row])
        return tmp + '</table>'
